...
class trading {
	...
	bookZoomAdd() {
		for (let n = 0; n < 10; n++) {
			const increase = cy
				.get('.split__main > .ui-panel > .collapsible > .ui-collapsible__header')
				.get(
	...
	addAlert() {
		const setBidAlert = cy
			...
			.first('div')
			.each($div => {
				cy.get('#book-bids > .book__rows > :nth-child(1) > .book__alert > .fa').click()
			})
		...
		const setAskAlert = cy
			...
			.first()
			.each($div => {
				cy.get('#book-asks > .book__rows > :nth-child(1) > .book__alert > .fa').click()
			})
	...
	checkMaxValue() {
		...
		const compareBuy = cy
			...
			.type('USD', '{enter}')
			.get(
				'[data-qa-id="balancesTable"] > [style="overflow: visible; height: 0px; width: 0px;"] > [tabindex="-1"] > .custom-scrollbar > [style="height: 64px; width: 100%;"] > .table-vir__row-even > :nth-child(2) > .trigger-ledger-modal > :nth-child(2) > .trigger > .avail'
			)
			.then($val => {
				const txt = $val.text()
				var pointNum = parseFloat(txt)
				...
				const checkMaxAsk = cy
					.get(
						':nth-child(2) > .orderform__field > .ui-labeledinput__container > .ui-fieldlabel__container > .ui-buysellinputindicator'
					)
					.get(
	...
	increaseDecreasePrecision() {
		for (let n = 0; n < 2; n++) {
			const decrease = cy
				...
				.click()
			const bookDescreased = cy.get('#book-bids > .book__rows').should('be.visible')
		}
		const increase = cy
			.get(
				'.split__main > .ui-panel > .collapsible > .ui-collapsible__header > [style="visibility: visible;"]'
			...
			.click()
		const book = cy.get('#book-bids > .book__rows').should('be.visible')
		return this
	}
}
