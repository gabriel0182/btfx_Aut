class buyStop {
	...
	requiredFields() {
		...
		buy.click()
		const price = cy.get('.order-errors').get('.order-errors__wrapper').get('li')
		...
		testData.forEach(testDataRow => {
			...
			context(`Generating a test for ${data.wallet1}`, () => {
				...
				cy.get(':nth-child(2) > h5 > span').then($btn => {
					...
					var amout = pointNum * 1120
					var value = amout + 100
					const priceUSD = cy.get('[name="price"]').clear({ force: true }).type(value)
				})
				const amountBTC = cy.get('[name="amount"]')
		...
		testData.forEach(testDataRow => {
			...
			context(`Generating a test for ${data.price}`, () => {
				...
				const verifyMsg = cy.waitUntil(() =>
					cy
						.get('.notification-text__text')
						.should('contain', `Created exchange stop buy order of ${data.btc} BTC`)
				)
			})
		})
		return this
				...
				testData.forEach(testDataRow => {
					...
					context(`Generating a test for ${data.btc}`, () => {
						...
						const verifyMsg = cy.waitUntil(() =>
							cy
								.get('.notification-text__text')
								.should('contain', `Exchange stop buy order of ${data.btc} BTC has been canceled`)
						)
					})
				})
			})
