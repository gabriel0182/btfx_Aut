class stopLimitExch {
	...
	requiredFields() {
		...
		buy.click()
		const limitPrice = cy.get('.order-errors').get('.order-errors__wrapper').get('li')
		...
		const priceUSD = cy.get('.order-errors').get('.order-errors__wrapper').get('li')
		...
		testData.forEach((testDataRow) => {
			...
			context(`Generating a test for ${data.wallet1}`, () => {
				...
				cy.get('.main-ticker__items > :nth-child(6) > :nth-child(2)').then(($btn) => {
					...
					const limitUSD = cy.get('.orderform > :nth-child(4)').get('#priceinput5')
		...
		testData.forEach((testDataRow) => {
			...
			context(`Generating a test for ${data.wallet1}`, () => {
				...
				cy.get('.main-ticker__items > :nth-child(6) > :nth-child(2)').then(($btn) => {
					...
					const priceUSD = cy.get('[name="price"]').clear({ force: true }).type(txt)
					const amountBTC = cy.get('[name="amount"]')
					amountBTC.clear({ force: true }).type(data.btc)
					const limitUSD = cy.get('.orderform > :nth-child(4)').get('#priceinput5')
					limitUSD.clear({ force: true })
					limitUSD.type(value2)
					const hidden = cy
		...
		testData.forEach((testDataRow) => {
			...
			context(`Generating a test for ${data.price}`, () => {
				...
				const verifyMsg = cy.waitUntil(() =>
					cy
						.get('.notification-text__text')
						.should('contain', `Created exchange stop limit buy order of ${data.btc} BTC`)
				)
			})
		})
		return this
	...
	validateHidden() {
		const ordersTable = cy
			...
			.each(($div) => {
				...
				cy.get(
					'[style="flex: 1 1 90px; min-width: 90px;"] > .virtable__cellwrapper > ._3gvQcbWp-vbomwHFC_BrJY'
				)
