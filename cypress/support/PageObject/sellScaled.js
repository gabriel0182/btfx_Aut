class sellScaled {
	...
	successMsg() {
		...
		const verifyMsg = cy.waitUntil(() =>
			cy.get('.notification-text__text').should('contain', `Created exchange limit sell order of`)
		)
		return this
	}
	orderFilter() {
	...
	cancelOrder() {
		const msg = cy.waitUntil(() =>
			cy.get('.notification-text__text').should('be.visible').get('div.notification__skip').click()
		)
	...
	cleanFilters() {
		const filter = cy.get('div.ui-collapsible__header').within(() => {
			cy.get('a.filter-select__menu-btn')
				.get('[data-qa-id="orders-filter-icon"]')
				.first()
				.click({ force: true })
		})
		const reset = cy.get('div.ui-contextmenu__menu--left').get('.filter-select__reset-btn')
		reset.click()
		const selectAll = cy
			.get(
			...
			.get(
				'[style="flex: 0 1 105px; min-width: 105px; max-width: 105px;"] > :nth-child(1) > .ui-button > .fa'
			)
			.click()
			.get(
				'.table-vir__header > [style="flex: 0 1 105px; min-width: 105px; max-width: 105px;"] > :nth-child(2) > .ui-button'
			)
			.get(
				'.table-vir__header > [style="flex: 0 1 105px; min-width: 105px; max-width: 105px;"] > :nth-child(2) > .ui-button > .fa'
			)
			.click()
