class buyStopTIF {
	...
	buyButton() {
		const exchangeBuy = cy.waitUntil(() => cy.get('#buyButton').click())
		return this
	}
	successMsg() {
		const testData = require('../../fixtures/orders.json')
			...
			context(`Generating a test for ${data.price}`, () => {
				...
				const verifyMsg = cy.waitUntil(() =>
					cy
						...
						.should('contain', `Created margin stop buy order of ${data.btc} BTC`)
				)
			})
		})
		return this
				...
				testData.forEach(testDataRow => {
					...
					context(`Generating a test for ${data.btc}`, () => {
						...
						const verifyMsg = cy.waitUntil(() =>
							cy
								.get('.notification-text__text')
								.should('contain', `Margin stop buy order of ${data.btc} BTC has been canceled`)
						)
					})
				})
			})
